...
public class Search extends JFrame implements ActionListener {
	...
	*/
	...
	Search() {
		...
		//textpanel.add(title);
		lbhn = new JLabel("Hotel Name");
		tfhn = new JTextField(30);
		lbhn.setLabelFor(tfhn);
   	...
   	public void actionPerformed(ActionEvent ae){
			...
			try{
			String h_name = tfhn.getText();
			PreparedStatement pstmt1=f1.conn.prepareStatement("select * from hotel where h_no_assign < min_assign and h_name = ? ");
			pstmt1.setString(1, h_name);
			ResultSet rs = pstmt1.executeQuery();

			while(rs.next()){
							String name = rs.getString("h_name");
							String name2 = rs.getString("branch");
